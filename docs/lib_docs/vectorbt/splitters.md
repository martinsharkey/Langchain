...
## split_ranges_into_sets function&para;
...
Args
 `start_idxs` :&ensp;`array_like` Start indices. `end_idxs` :&ensp;`array_like` End indices. `set_lens` :&ensp;`list` of `float` 
...
Can be passed per range.
 `left_to_right` :&ensp;`bool` or `list` of `bool` 
