...
class PatternMatcher:
    ...
    def analyze_current_market(self, indicators: dict, symbol: str = "") -> dict:
        """
        ...
        """
        # Find similar patterns (both winning and losing), filtered by base symbol
        # to avoid cross-symbol contamination (BTC patterns must not influence gold).
        similar = self.vector_store.find_similar(indicators, n_results=10, symbol=symbol)
        winning = self.vector_store.find_winning_patterns(indicators, n_results=5, symbol=symbol)
        losing = self.vector_store.find_losing_patterns(indicators, n_results=5, symbol=symbol)
    ...
    def get_historical_confidence(
        ...
        Args:
            ...
            symbol: Base symbol to filter patterns by.
        
        Returns:
            Confidence adjustment (-0.3 to +0.3).
        """
        if strategy_name:
            # Find similar patterns where this strategy was used, filtered by symbol
            similar = self.vector_store.find_similar(indicators, n_results=20, symbol=symbol)
            strategy_patterns = [
                p for p in similar
    ...
    def get_strategy_recommendations(self, indicators: dict, symbol: str = "") -> list[dict]:
        """
        ...
        """
        similar = self.vector_store.find_similar(indicators, n_results=30, symbol=symbol)
    ...
    def find_optimal_strategy_combination(
        ...
        """
        recommendations = self.get_strategy_recommendations(indicators, symbol=symbol)
