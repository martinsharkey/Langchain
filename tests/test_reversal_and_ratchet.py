...
def test_signature_from_captured_reads_snapshots():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        # osma grew entry->peak then shrank peak->exit (toward neutral)
        assert sig["osma"]["entry_to_peak_median"] > 0
        assert sig["osma"]["peak_to_exit_median"] < 0
...
def _proven_signature(shrink_pct=80.0):
    """A signature where osma + macd_histogram reliably shrink toward neutral at exit."""
    return {
        "osma": {"shrank_toward_neutral_pct": shrink_pct, "peak_to_exit_median": -0.6, "n": 30},
        "macd_histogram": {"shrank_toward_neutral_pct": shrink_pct, "peak_to_exit_median": -0.3, "n": 30},
